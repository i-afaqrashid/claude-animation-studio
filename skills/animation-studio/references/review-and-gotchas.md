...
✓ in sync
```
Reading it: sound ±20 ms and picture ±1 frame pass. The picture counts the pixels that change a lot from one frame to the next, so cuts, flashes, stamps and pops stand out while fades and grain don't. It can land one frame late after a white flash (the flash frame differs less from a bright scene than the frame after it does) or one frame early when motion accelerates INTO the hit (a lid slamming shut). ✗ = a real offset: move the event in `score.js` so both sides read the same time. `?` = unclear, and it FAILS unless you pass `--allow-unclear`. Either there's no distinct hit near the marker (a small pop in a busy frame, a note buried in the mix), or two sound onsets are almost equally steep and more than 40 ms apart. Verify then prints both times, because the file can't tell which one the marker means (a sung pickup before a sung downbeat; the last stroke of a drum roll before the hit). Make the moment clearer (a real transient, an 8th of held breath before it), or give it `sync: 'a'` / `'v'` / no sync. Run verify on the files you publish (after any re-encode), not only on the render.
...
| A marker on a sung downbeat verifies on the render but fails (−145 ms) on the re-encoded release file | the sung pickup one 8th earlier is almost as steep an onset, and the AAC re-encode tipped the tie | verify now reports such near-ties as `?` with both times (a fail unless `--allow-unclear`); put sync markers on real hits, not on legato singing (birthday-card) |
